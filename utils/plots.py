...
def plot_one_circle(cx, cy, r, img, color=None, label=None, line_thickness=None):
    """
    绘制一个圆形框
    """
    line_thickness = line_thickness or max(round(sum(img.shape) / 2 * 0.003), 1)
    color = color or [0, 255, 0]  # 默认颜色为绿色
    cv2.circle(img, (int(cx), int(cy)), int(r), color, thickness=line_thickness, lineType=cv2.LINE_AA)

    if label:
        font_thickness = max(line_thickness - 1, 1)
        t_size = cv2.getTextSize(label, 0, fontScale=line_thickness / 3, thickness=font_thickness)[0]
        c2 = (cx - r, cy - r - t_size[1] - 3)
        cv2.rectangle(img, (cx - r, cy - r - t_size[1] - 3), c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img, label, (cx - r, cy - r - 2), 0, line_thickness / 3, [225, 255, 255], thickness=font_thickness,
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    plot_one_circle(cx, cy, r, mosaic, color=color, label=label, line_thickness=fs // 10)
