...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        if len(cache) == 0:
            raise Exception(f"{prefix}Error reading cache from {cache_path}, cache is empty")
    ...
    def load_labels(self, label_path):
        ...
        with open(label_path, 'r') as file:
            for line in file.readlines():
                class_id, cx, cy, radius = map(float, line.split())
                # 存储类别，圆心x，圆心y，半径
                labels.append([class_id, cx, cy, radius])
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        ...
        LOGGER.info(f"{prefix}Starting to cache labels at {path}")
        ...
        pbar.close()
        LOGGER.info(f"{prefix}Finished scanning. Found: {nf}, Missing: {nm}, Empty: {ne}, Corrupt: {nc}")
        ...
        # 检查缓存字典是否为空
        if not x:
            LOGGER.error(f"{prefix}Error: Cache dictionary is empty before saving to {path}")
        else:
            LOGGER.info(f"{prefix}Cache dictionary is ready to save with {len(x)} entries")

        LOGGER.info(f"{prefix}Saving cache to {path}")
        try:
    ...
    def __getitem__(self, index):
        ...
        return img_tensor, labels, img_path, img_tensor.shape
        ...
        if im_file.suffix[1:] in IMG_FORMATS:
            ...
            if Path(lb_file).exists():
                with open(lb_file) as f:
                    lb = np.array([x.split() for x in f.read().strip().splitlines() if len(x)], dtype=np.float32)
