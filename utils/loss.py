...
def circle_iou(cx1, cy1, r1, cx2, cy2, r2):
    """计算两个圆形之间的IoU"""
    d = torch.sqrt((cx1 - cx2) ** 2 + (cy1 - cy2) ** 2)  # 圆心距离
    r1, r2 = r1.squeeze(), r2.squeeze()

    # 判断两个圆是否相交
    if d >= r1 + r2:
        return torch.tensor(0.0)  # 不相交
    elif d <= torch.abs(r1 - r2):
        return torch.tensor(1.0)  # 一个圆包含在另一个圆内

    # 计算重叠面积
    r1_square, r2_square = r1 ** 2, r2 ** 2
    part1 = r1_square * torch.acos((d ** 2 + r1_square - r2_square) / (2 * d * r1))
    part2 = r2_square * torch.acos((d ** 2 + r2_square - r1_square) / (2 * d * r2))
    part3 = 0.5 * torch.sqrt((-d + r1 + r2) * (d + r1 - r2) * (d - r1 + r2) * (d + r1 + r2))

    overlap_area = part1 + part2 - part3
    union_area = torch.pi * (r1_square + r2_square) - overlap_area
