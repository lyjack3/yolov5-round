...
def train(hyp, opt, device, callbacks):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Forward
            # 在训练循环中，使用 compute_loss 计算损失
            with torch.cuda.amp.autocast(amp):
                pred = model(imgs)  # forward
                loss = compute_loss(pred, targets.to(device))  # 使用 CircleLoss 计算损失
                if RANK != -1:
...
def parse_opt(known=False):
    ...
    parser.add_argument("--weights", type=str, default=ROOT / "model/weights/yolov5n.pt", help="initial weights path")
    parser.add_argument("--cfg", type=str, default="yolov5n.yaml", help="model.yaml path")
    parser.add_argument("--data", type=str, default=ROOT / "data/Google-oiltank.yaml", help="dataset.yaml path")
    ...
    parser.add_argument("--name", default="OiltankDetection_20240605", help="save to project/name")
